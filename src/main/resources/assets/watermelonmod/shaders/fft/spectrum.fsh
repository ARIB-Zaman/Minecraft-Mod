...
void main() {
    ...
    // Params.x > 1 zooms into the low frequencies around DC.
    vec2 uv = 0.5 + (texCoord - 0.5) / max(Params.x, 1.0);
    vec4 rg = texture(RGSampler, uv);
    vec4 blue = texture(BSampler, uv);
    ...
    float normalisedMagnitude = magnitude / (InSize.x * InSize.y);
