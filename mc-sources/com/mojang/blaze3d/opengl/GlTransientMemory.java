	...
	static class Fallback extends GlTransientMemory {
		...
		public GpuBufferSlice.MappedView allocateStaging(final long size, final long alignment, final int usage, final long minimumAllocation, final long elementSize) {
			...
			return new GpuBufferSlice.MappedView(
				...
				() -> this.dsa
					.bufferSubData(
						transientBuffer.handle(), slice.offset(), MemoryUtil.memByteBuffer(allocation.block().hostBuffer + allocation.offset(), (int)allocation.size()), usage
					)
			);
		}
	...
	static class PersistentMapping extends GlTransientMemory {
		...
		PersistentMapping(final GlDevice device, final GlCommandEncoder encoder) {
			...
			this.stagingBlockAllocator = new TransientBlockAllocator<>(
				524288L, GlTransientMemory.MAX_GPU_ALIGNMENT, TransientBlockAllocator.Allocator.create(size -> this.allocateGlBlock(size, true, true), this::freeGlBlock)
			);
			this.gpuBlockAllocator = new TransientBlockAllocator<>(
				524288L, GlTransientMemory.MAX_GPU_ALIGNMENT, TransientBlockAllocator.Allocator.create(size -> this.allocateGlBlock(size, false, false), this::freeGlBlock)
			);
			this.gpuMappedBlockAllocator = new TransientBlockAllocator<>(
				524288L, GlTransientMemory.MAX_GPU_ALIGNMENT, TransientBlockAllocator.Allocator.create(size -> this.allocateGlBlock(size, false, true), this::freeGlBlock)
			);
		}
