...
public class RealmsPlayersTab extends GridLayoutTab implements RealmsConfigurationTab {
	...
	public RealmsPlayersTab(final RealmsConfigureWorldScreen configurationScreen, final Minecraft minecraft, final RealmsServer serverData) {
		...
		helper.addChild(
			Button.builder(
					Component.translatable("mco.configure.world.buttons.invite"), var3 -> minecraft.gui.setScreen(new RealmsInviteScreen(configurationScreen, serverData))
				)
				.build(),
			...
			if (index >= 0 && index < RealmsPlayersTab.this.serverData.players.size()) {
				...
				RealmsConfirmScreen confirmScreen = new RealmsConfirmScreen(
					result -> {
						if (result) {
							RealmsUtil.runAsync(
								client -> client.uninvite(RealmsPlayersTab.this.serverData.id, playerInfo.uuid), e -> RealmsPlayersTab.LOGGER.error("Couldn't uninvite user", e)
							);
